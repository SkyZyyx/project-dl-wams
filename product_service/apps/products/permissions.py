from rest_framework.permissions import BasePermission, SAFE_METHODS


def _token_claim(request, claim_name: str) -> str | None:
    user = request.user
    token = getattr(user, "token", None)
    if token is not None:
        claim = token.get(claim_name)
        if isinstance(claim, str) and claim:
            return claim
    return None


def _user_role(request) -> str | None:
    user = request.user
    role = getattr(user, "role", None)
    if isinstance(role, str) and role:
        return role
    return _token_claim(request, "role")


def _is_admin(request) -> bool:
    return _user_role(request) == "admin"


def _is_seller(request) -> bool:
    return _user_role(request) == "seller"


def _username(request) -> str:
    username = getattr(request.user, "username", "")
    if isinstance(username, str) and username:
        return username
    return _token_claim(request, "username") or ""


class SellerOrAdminWritePermission(BasePermission):
    def has_permission(self, request, view):
        if request.method in SAFE_METHODS:
            return True
        return _is_admin(request) or _is_seller(request)


class ProductOwnerOrAdminPermission(BasePermission):
    def has_object_permission(self, request, view, obj):
        if request.method in SAFE_METHODS:
            return True
        if _is_admin(request):
            return True
        if _is_seller(request):
            return bool(obj.seller_username) and obj.seller_username == _username(request)
        return False
